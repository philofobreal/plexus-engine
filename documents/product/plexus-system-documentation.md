...
* `cosmic-wormhole`: determinisztikus 3D grain-mezo. A konstruktorban allokalt szemcsek immutable normalizalt depth phase-t es stabil flow/kick/LOW_DROP karaktert tartanak. A travel koordinata a kanonikus dalidobol es fix analysis-hop integralbol szarmazik, ezert FPS, pause es seek history nem valtoztatja meg a geometriat. A kick timestamp-alapu, low-band tamogatast igenyel; a LOW_DROP hat determinisztikus lokalis toresvariansbol valaszt. A lassu depth/density/perspective evolucio nem mozgat kamerat vagy horizontot. A `wormholeWarp` es `wormholeCurve` lokalis grain-flow parameterek, nem route-geometria. A `wormholePathBend` a route heading/curvature intenzitasat vezerli a roll nelkuli camera-local travel frame-ben; nem lateral csodeformacio es nem canvas-forgatas. Automation presetvaltas kozben egy rovid `automationResponse` envelope live skalazza a path bend / speed / continuity reakciot, mikozben a grain radius/depth/ring/coherence tovabbra is release-snapshotolt marad. A depth-aware alpha floor a `lineAlpha` es emission gain alatt marad, igy nulla opacity teljesen kikapcsolja az effektet.
...
* wormhole immutable depth-phase egyenletesseget, fix-hop/FPS/live-export determinisztikussagot, timestamp-kick es LOW_DROP cohort szerzodest, lassu evolucios korlatokat, alpha-szerzodest, 100 seek utani serulesmentesseget es azonos dalpozicion reprodukalhato geometriat (`tests/wormhole-determinism.test.mjs`, `tests/wormhole-depth-integrity.test.mjs`, `tests/wormhole-lifecycle.test.mjs`).
