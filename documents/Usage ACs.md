...
*   **AC 1.4 - Élő Keresés és Scrub-pufferelés (Seek/Scrub):** A csúszka vagy a dramaturgiai timeline húzásakor az idő-kijelző és a vizuális lejátszófej valós időben frissül, de a hangmotor nem kap folyamatos `seek()` hívásokat. A UI a célpozíciót `scrubTime` állapotban puffereli, majd az egér/ujj elengedésekor egyetlen végleges `AudioEngine.seek()` hívással ugrik a kiválasztott időpontra.
...
*   **AC 9.5 - Interactive Dramaturgy Timeline:** The seekbar chrome includes a canvas timeline that visualizes precomputed `TrackAnalysis` data only. `drawTimelineGridlines` renders BPM-derived bar lines, section blocks use label-specific colors, `buildupConfidence` appears as a cyan wave, `tensionTrends` appear as directional guide strokes, RMS/bar dynamics appear as a secondary layer, significant cue types appear as markers, and the playhead is rendered from timeline state without runtime music analysis.
...
*   **AC 9.7 - Canvas Backend Hot-Path Optimization:** Particle boundary pull uses squared-distance checks and vector normalization instead of p5 distance and angle trigonometry. The p5 backend caches repeated fill, stroke, and stroke-weight state changes while preserving `noStroke`/`noFill` behavior.
*   **AC 9.8 - Fullscreen Overlay Structure:** Opening the timeline overlay applies `.timeline-overlay-active` to the full `.seek-container`, `.is-fullscreen-overlay` to `.timeline-wrapper`, and `body.timeline-overlay-open` to the document body. The timeline detaches from the seekbar grid, fills the viewport above other UI, hides unrelated chrome, and closes back to its previous bottom position and stored height.
*   **AC 9.9 - HTML Tooltip Interaction:** Hovering the timeline shows `#timeline-tooltip` near the pointer. The tooltip reports the current time and zoom, matching section, bar index and `HIGH`/`LOW` state, RMS, B/M/T ratios, buildup pressure, tension trend, and nearby cue data where available.
*   **AC 9.10 - DAW-Style Zoom And Pan:** Mouse wheel zooms the timeline from `1x` to `16x` around the cursor position. Normal left click or drag always scrubs/seeks the playhead, including when zoomed. Shift-drag or middle-button drag pans the visible viewport. During playback, zoomed view follows the playhead when it leaves the `15%..75%` viewport range.
*   **AC 9.11 - Performance Line And Scrub Buffering:** Timeline and seekbar dragging must not repeatedly call the audio engine. Dragging updates `scrubTime`, the visible time, seekbar value, and a yellow scrub playhead through `requestTimelineDraw()`. A single final audio seek is committed on `pointerup`, `pointercancel`, `change`, or equivalent touch-end interaction.
