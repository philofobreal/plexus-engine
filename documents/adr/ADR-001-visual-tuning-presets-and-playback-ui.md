...
- **Continuous wormhole transition behavior:** `wormholeEmissionMode` participates in numeric tuning morphs. Fractional mode values crossfade adjacent deterministic emission envelopes. The parameter morph is the only wormhole preset-transition channel; it creates no separate shared camera or field transform.
