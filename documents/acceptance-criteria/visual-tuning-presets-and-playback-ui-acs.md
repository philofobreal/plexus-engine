...
- **VT-2.11 Wormhole group:** A `Wormhole` control group exposes `wormholeRadius`, `wormholeDepth`, `wormholeSpeed`, `wormholeWarp`, `wormholeCurve`, `wormholeRing`, `wormholeStarfield`, and `wormholeGalaxy`. `wormholeCurve` is a `0..1` master that can fully zero the tunnel curvature independent of preset content; `wormholeStarfield` and `wormholeGalaxy` are general background masters the bundled presets do not write, so they remain global across preset changes. New parameters follow VT-1.5 (typed default config plus control metadata, no duplicated slider UI).
...
- **VT-9.13 Performance Automation Plan display:** The timeline renders automation zones from `State.editedPerformancePlan` (falling back to `State.performancePlan` when no edits exist). Partially visible automation zones must not disappear at timeline viewport edges. Each `PerformanceAutomationPoint` zone shows a draggable vertical start handle, a morph-end resize handle, a horizontal sensitivity handle, and a shaded morph curve area for whole-zone sliding. `intensity` maps to the local sensitivity override (`0.1..4.0`). Morph curve math uses unclipped x coordinates and canvas clipping; the segment count is at least `15` and grows with the visible curve width. Preset-derived colors mark the automation zone, morph curve, intensity line, and sensitivity handle state, with the zone segment after the morph drawn dimmed. RMS/bar lines must remain connected at viewport edges. During the draw frame, `PlexusRenderer` applies the active point's `intensity` over the global `audioSensitivity` and restores the global value before the frame ends. Optional analyzer scheduling metadata (`analysisConfidence`, `timingMode`) must not break rendering or plan validation; `timingMode` accepts `bar-aligned`, `novelty`, and `energy-reactive`. `State.sectionOverrides` is fully removed.
...
- **VT-9.23 Deterministic choreography provenance:** Generation is fully deterministic — identical `TrackAnalysis` + settings produce a byte-identical plan — using a seed hashed from the analysis (`hash(trackSeed, sceneIndex, segmentIndex)`), with no runtime randomness. Each `PerformanceAutomationPoint.meta` carries renderer-independent provenance (`automationSituation`, `vocabularyId`, `variantRole`, and the opaque `targetStateReference`), surfaced in the timeline debug tooltip and round-tripped by Copy/Load. The planner names no preset filename or tuning key. Breathing, adaptive bar-snapped subdivision, cycle grammar, weighted memory, determinism, Stable-edge variation, and Variation-vs-Activity separation are covered by `tests/visual-os.test.mjs`.
...
- **VT-10.9 Cosmic Wormhole identity:** The `cosmic-wormhole` identity must drive a 3D perspective tunnel from a constructor-allocated dust pool, the 24-band `perceptualSpectrum`, and the modulation bus, drawing only through `VisualRendererBackend` (`backend.line` plus a `shouldUseExpensiveGlow`-gated galaxy `radialGlow`) with no per-frame allocation and deterministic `pseudoNoise()` motion. Tunnel curvature must be event-driven and scaled by the `wormholeCurve` master so a value of `0` yields a straight tube regardless of preset content. Grain recycling must preserve depth dispersion (no collapse into rings after a full play-through), with `wormholeRing` as the opt-in dispersion-to-rings control. The background starfield and galaxy parallax must follow camera travel and curvature, and `wormholeStarfield`/`wormholeGalaxy` remain general, preset-independent masters.
