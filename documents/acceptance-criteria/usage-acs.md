...
* **AC 5.7 - Evidence-based section labels:** `SectionAnalyzer` labels sections by scoring every supported label with normalized evidence from energy, previous section energy, density, bass/density context, tension, dominant feature, and section position. It must not depend on a rigid label if/else threshold chain; weak evidence falls back to `verse`.
* **AC 5.8 - Fuzzy pattern grouping:** `DramaturgyBuilder` groups recurring `MusicPattern` candidates with deterministic vector similarity. Energy, density, and dominant feature are compared with Euclidean distance against group centroids, and matches below the configured threshold join the existing motif group instead of requiring exact string signature equality.
