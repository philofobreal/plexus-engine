...
* **AC 9.14 - Timeline automation generator:** The timeline automation generator can create a `PerformanceAutomationPlan` from three strategies: `Dramaturgy` (music-aware default), `Hero Rhythm` (maps sections to `heroBeepMode` rhythms), and `Strict Alternating` (ignores music sections and alternates the chosen presets strictly on the musical bar grid). Generating a new plan confirms before overwriting an existing edited or generated plan.
