...
- The golden snapshots are generated by `bun run update-golden-masters` (`scripts/generate-golden-masters.ts`). This command intentionally OVERWRITES baselines and must be run ONLY when a timing change is a deliberate, reviewed improvement. Inspect the git diff before committing regenerated snapshots.
