...
- `SpectralCalibration`: performs a deterministic offline pre-pass over fixed-stride FFT windows, estimates track-level spectral centers within safety ranges, and falls back to the default Hz bands for silence or low-confidence material.
...
- `FeatureClassifier`: maps normalized low-level DSP vectors into semantic music scores for melody, vocal, fx, density, brightness, and tension. FX scoring uses Zero Crossing Rate, flatness, rolloff, brilliance, and air energy; melody and vocal are spectral heuristics based on tonal stability and low/mid/presence bands, not stem separation.
