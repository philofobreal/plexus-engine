...
- `cosmic-wormhole`: a 3D perspective "tunnel flight" identity. A constructor-allocated dust pool is projected from cylinder space to 2D using the 24-band `perceptualSpectrum` and the modulation bus. Grains keep immutable normalized depth phases and one shared travel phase; depth is derived against the current horizon, so depth morphs cannot accumulate per-grain distribution damage. `wormholeDepthCoherence` adds deterministic authored cohort compression without reintroducing mutable depths, while `wormholeRing` remains a separate ring-alignment control. `syncPosition()` re-anchors travel, automation transition, starfield, and galaxy state after playback discontinuities. The identity adds event-driven tunnel curvature scaled by `wormholeCurve`, an absolute-world parallax starfield, and a deep `radialGlow` galaxy layer. It draws only `backend.line` plus the gated galaxy glow and stays deterministic through `pseudoNoise()`. `wormholeEmissionMode` is continuous during tuning morphs: `wormholeEmissionGain()` evaluates adjacent integer modes and crossfades their gains for fractional values. `WormholeAutomationTransition` tracks automation identity changes separately from tuning floats and returns a zero-start smoothstep response over `State.targetTuning.morphDurationSec`, avoiding an instant first-frame curvature surge.
