...
### Renderer post FX chain

`PostFxPipeline.render()` runs once per `draw()`, immediately after the identity draw branch, so it processes the finished frame in both the steady-state and the crossfade path and never once per participating identity. It bypasses everything when `performanceMode >= 0.5` or when the renderer is neither playing nor exporting, then resolves the destination (`__plexusExportTarget` when exporting, the live canvas otherwise), probes each effect, and only on an active frame copies the destination into the single persistent buffer owned by `CanvasPostFxSurface`. The buffer is created on the first active frame and resized only when destination dimensions change. `P5RenderTargetCompositor` is untouched and keeps owning identity replacement only.

`TemporalFragmentationEffect` is the first effect. `State.directorOutput.glitchIntensity` owns the accent lifecycle: it snaps to `1.0` on `GLITCH_LOW_DROP` entry, decays as `exp(-4t)`, and the effect is fully bypassed below `0.12`, which bounds an accent to roughly half a second. `spectralChaos` shapes fragmentation complexity only and never gates, so sustained chaos cannot produce a continuous glitch; `rhythmicImpulse` scales the one-shot displacement within bounds. `temporalFragmentationPlan.ts` is a pure planner over song time, the published signals, tuning, and surface size, using an integer avalanche hash instead of runtime randomness, so live playback, seek, and export produce the same decisions at the same song time. Band boundaries and spans come from a `0.75s` topology grid and the moving selection from a `0.18s` activation grid, which keeps one accent on one fragmentation while it re-jumps a few times internally. Displacement is X-only with same-size, same-Y source rects plus a wrap copy inside a clip, so transparent, chroma-key, and video-backplate output keep exact pixels and alpha with no hole in either axis. `postFxFragmentAmount` defaults to `0`, so the default output is unchanged.
