...
The headless analyzer derives these values from a fixed 1024-sample FFT pipeline in `src/analyzer/`. `analyzeAudio()` is the single orchestration implementation and now follows an explicit `Extractor -> Normalizer -> Classifier -> Smoother` pipeline before section and dramaturgy analysis:

- `FeatureExtractor`: performs Hann-windowed FFT analysis and produces per-frame RMS, spectral flux, band ratios, centroid, flatness, pitch confidence, Zero Crossing Rate, 85% spectral rolloff, spectral crest, and typical maxima.
...
- `FeatureClassifier`: maps normalized low-level DSP vectors into semantic music scores for melody, vocal, fx, density, brightness, and tension. FX scoring uses Zero Crossing Rate, flatness, rolloff, and high-band ratio; melody and vocal scoring use tonal stability, spectral crest, mid/high balance, and ZCR rejection for noisy material.
...
Each frame is Hann-windowed before the FFT, then `FeatureExtractor` calculates spectral flux, relative bass/mid/high magnitude bands, spectral centroid, spectral flatness, Zero Crossing Rate, 85% spectral rolloff, and spectral crest. `FeatureClassifier` combines these low-level features into semantic projections: high ZCR/flatness/rolloff favors FX, strong crest plus low flatness favors melody, and mid-band formant balance with low ZCR favors vocal. The render-facing `AudioFrame` values remain schema-compatible semantic projections: `e` is normalized RMS energy, `densityProj` is smoothed spectral-flux density, `melodyProj` is smoothed tonal melody presence, and `fxProj` is smoothed FX/noise/transient pressure. Beat events are selected from spectral-flux peaks and classified through `BeatEventClassifier`: generic and kick-like impacts map to type 1, dense impacts map to type 2, and high-ZCR/high-rolloff transients map to type 3.
