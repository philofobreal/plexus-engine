...
- `BeatEventClassifier`: classifies a beat into an internal semantic hit kind using bass ratio, Zero Crossing Rate, rolloff, and high-band context at the beat frame, then maps it back to the public `1 | 2 | 3` schema.
- `DramaturgyBuilder`: emits `BeatEvent`, `VisualCueEvent`, and recurring `MusicPattern` outputs. Beat events are derived from the authoritative `GridAligner.beats`, not an independent peak-picker; a beat whose local onset energy is negligible (an extrapolated silent/breakdown beat) is suppressed as a visual event so the renderer is not flooded with phantom flashes, while the grid itself still spans the silence. `VisualCueEvent` and `MusicPattern` use fuzzy vector similarity for grouping.
