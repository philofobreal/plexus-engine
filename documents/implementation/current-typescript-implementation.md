...
- `FeatureExtractor`: performs Hann-windowed FFT analysis and produces per-frame RMS, spectral flux, Hz-based band ratios, centroid, flatness, pitch confidence, Zero Crossing Rate, 85% spectral rolloff, spectral crest, and typical maxima.
...
- `GridAligner`: estimates BPM, builds ordered tempo candidates, resolves close half/double-time aliases, projects a beat/bar grid, and publishes `gridOffset` plus BPM, grid, and downbeat confidence.
...
- `BeatEventClassifier`: classifies accepted spectral-flux peaks into internal semantic hit kinds using bass ratio, Zero Crossing Rate, rolloff, and high-band context, then maps them back to the public `1 | 2 | 3` schema.
...
`GridAligner` confidence values are evidence-capped where the evidence actually applies. `bpmConfidence` combines histogram dominance and candidate support, then caps the result by interval evidence and a low/kick-transient support cap. The cap is intentionally named narrowly in code: it measures support from RMS rise and low-band/bass content, so it should not be read as universal rhythm confidence for bass-light material. `gridConfidence` is based on onset-to-grid match ratio and timing error, with BPM confidence as a conservative weighting and cap; it is not multiplied by low-end transient loudness, but it also cannot drift too far above weak BPM evidence. `downbeatConfidence` is capped by both `gridConfidence` and `bpmConfidence * 1.2`, so a weak tempo/grid cannot publish a strong downbeat. The half/double-time resolver annotates aliases such as 85/170 or 64/128 and may reorder only close-score alias candidates; a clearly dominant histogram peak remains first.

Each frame is Hann-windowed before the FFT, then `FeatureExtractor` calculates spectral flux, relative Hz-band magnitude ratios, spectral centroid, spectral flatness, Zero Crossing Rate, 85% spectral rolloff, and spectral crest. `FeatureClassifier` combines these low-level features into semantic projections: high ZCR/flatness/rolloff favors FX, strong crest plus low flatness favors melody presence, and low-mid/mid/presence balance with low ZCR favors vocal-like mid/presence dominance. Vocal is not lyric or singer detection; FX is transient/high-frequency presence, not an effects bus. None of these metrics are AI/ML stem separators. The render-facing `AudioFrame` values remain schema-compatible semantic projections: `e` is normalized RMS energy, `densityProj` is smoothed spectral-flux density, `melodyProj` is smoothed tonal melody presence, and `fxProj` is smoothed FX/noise/transient pressure. Beat events are selected from spectral-flux peaks and classified through `BeatEventClassifier`: generic and kick-like impacts map to type 1, dense impacts map to type 2, and high-ZCR/high-rolloff transients map to type 3.
...
- **AC 3.6 - Beat event classification:** Beat type classification is part of the analyzer contract: `BeatEventClassifier` uses bass ratio, Zero Crossing Rate, spectral rolloff, high-band ratio, and normalized flux to choose an internal hit kind, then maps it back to the public schema. Generic and kick-like impacts emit type 1 (`default spectral-flux hit`), dense impacts emit type 2 (`dense impact hit`), and high-ZCR/high-rolloff transients emit type 3 (`fx/high-transient hit`).
