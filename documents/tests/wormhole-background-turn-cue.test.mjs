...
test('background turn cue reacts to a live wormholePathBend change immediately, without waiting for a grain generation release', () => {
  const load = createSourceLoader();
  const { CosmicWormholeIdentity } = load('visuals/CosmicWormholeIdentity.ts');
  const { State } = load('state/store.ts');
  const spiral = JSON.parse(readFileSync(
  ...
  assert.ok(
    shift > 1,
    `expected the background star to move immediately once live wormholePathBend changed, got a ${shift.toFixed(3)}px shift`
  );
});
