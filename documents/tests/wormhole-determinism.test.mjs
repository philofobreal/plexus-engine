...
test('camera-relative route frame is an exact zero-bend baseline', () => {
  ...
  assert.match(source, /sampleWormholeRouteFrame\(distanceNow \+ z, effectivePathBend, this\.routeNow\)/);
  assert.match(source, /sampleWormholeRouteFrame\(distanceNow, effectivePathBend, this\.baseRouteNow\)/);
...
test('background layers use the same route-local travel frame, not screen rotation', () => {
  const source = readFileSync(join(SRC_ROOT, 'visuals/CosmicWormholeIdentity.ts'), 'utf8');
  assert.match(source, /sampleWormholeRouteFrame\(camZ, effectivePathBend, this\.baseRouteNow\)/);
  ...
  assert.match(source, /sampleWormholeRouteFrame\(starPrevCamZ, effectivePathBend, this\.baseRoutePrev\)/);
  assert.match(source, /sampleWormholeRouteFrame\(camZ \+ z, effectivePathBend, this\.routeNow\)/);
  assert.match(source, /const starLateralX = star\.x \* STAR_ROUTE_WORLD_SCALE \* starParallax \* this\.routeNow\.normalX/);
  assert.match(source, /const prevStarLateralX = star\.x \* STAR_ROUTE_WORLD_SCALE \* starParallaxPrev \* this\.routeNow\.normalX/);
  assert.match(source, /const galaxyDepthTravel = camZ \* GALAXY_SPEED_RATIO/);
  assert.match(source, /sampleWormholeRouteFrame\(camZ \+ gz, effectivePathBend, this\.routeNow\)/);
...
test('grain projection samples real current and previous route positions without moving the lens', () => {
  ...
  assert.match(source, /sampleWormholeRouteFrame\(distanceNow \+ z, effectivePathBend, this\.routeNow\)/);
  assert.match(source, /sampleWormholeRouteFrame\(distancePrev \+ prevZ, effectivePathBend, this\.routePrev\)/);
  assert.match(source, /sampleWormholeRouteFrame\(distanceNow, effectivePathBend, this\.baseRouteNow\)/);
  assert.match(source, /sampleWormholeRouteFrame\(distancePrev, effectivePathBend, this\.baseRoutePrev\)/);
...
test('route sampling and draw-time grain projection do not allocate path or grain objects', () => {
  ...
  assert.match(drawBody, /sampleWormholeRouteFrame\([^;]+, this\.routeNow\)/);
  assert.match(drawBody, /sampleWormholeRouteFrame\([^;]+, this\.routePrev\)/);
});
...
test('tunnel geometry has a canonical phase and no live or oscillating position multiplier', () => {
  ...
  assert.match(source, /const vz = wormholeForegroundTravelRate\(effectiveTravelRate\);/);
