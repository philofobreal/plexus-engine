...
const WORMHOLE_FACTORY_PRESET_KEYS = WORMHOLE_TUNING_KEYS.filter((key) => !WORMHOLE_BACKGROUND_MASTER_KEYS.includes(key));
...
test('clip roles have materially different wormhole behaviour (role-level contrast)', () => {
  ...
  assert.ok(p.punch.wormholePathBend > p.galaxy.wormholePathBend, 'punch uses a tighter impact arc than galaxy');
  assert.ok(p.overdrive.wormholePathBend > p.punch.wormholePathBend, 'overdrive intensifies the impact arc');
  ...
  assert.ok(p.collapse.wormholePathBend <= p.drift.wormholePathBend, 'collapse route stays restrained');
  assert.ok(p.sparse.wormholePathBend <= p.drift.wormholePathBend, 'sparse route stays restrained');
  for (const role of WH_ROLES) {
...
test('authored wormhole preset pairs form the requested contrast matrix', () => {
  ...
  assert.ok(p.galaxy.wormholeContinuity > p.sparse.wormholeContinuity, 'sparse -> galaxy deepens streak continuity');
