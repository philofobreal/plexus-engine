...
The prior legacy dashboard labels were also cleaned up: `AudioFrame.b`, `AudioFrame.m`, and `AudioFrame.t` now remain TypeScript legacy compatibility projection fields while the user-facing labels are `Density`, `Melody Presence`, and `FX Presence`. The `AudioFrame` type comments explicitly document `e` as normalized energy, `b` as density projection rather than bass, `m` as melody-presence projection rather than mid band, and `t` as FX-presence projection rather than treble.
...
| `AudioFrame.b` | `src/audio/analyzer.worker.ts` | worker `onmessage`, pass 2 frame loop | Output is `sDensity`, where `sDensity` tracks `clamp01(normFlux)` and `normFlux = fluxT[i] / typFlux`; despite local `sB` tracking raw bass, `sB` is not output | Intended `0..1`; `normFlux` may exceed 1 before `clamp01` target | `sDensity += (clamp01(normFlux) - sDensity) * 0.15`; paused/stopped decay `* 0.9` on render copy only | `State.currentFrame`, dashboard, `writeModulationBus` | `Density` | `densityDrive = scaleUnit(frame.b * 0.62 + features.density * 0.24 + frame.e * 0.14)`; drives particle motion, glow size/alpha, network density, temporal rings | User-facing label fixed; internal field remains legacy/compatibility | Medium |
| `AudioFrame.m` | `src/audio/analyzer.worker.ts` | worker `onmessage`, pass 2 frame loop | Output is `sMelody`; `melodyTarget = tonalFactor * max(0, rawMid - 0.2) * 1.35 + pitchedTransient * 1.4`; `tonalFactor = clamp01((1 - min(1, flatness * 1.8)) * 0.72 + harmonicStability * 0.28)`; `pitchedTransient = clamp01(pitchConfidence * max(0, rawMid - 0.12) * min(1, transientRatio * 4))` | Intended `0..1` | `sMelody += (clamp01(melodyTarget) - sMelody) * 0.1`; paused/stopped decay `* 0.9` on render copy only | `State.currentFrame`, dashboard, `writeModulationBus` only through feature duplicate when current features are also published | `Melody Presence` | Not directly read by effect modules; contributes through modulation only if logic later uses `frame.m` | User-facing label fixed; internal field remains legacy/compatibility | Medium |
| `AudioFrame.t` | `src/audio/analyzer.worker.ts` | worker `onmessage`, pass 2 frame loop | Output is `sFx`; `fxTarget = noiseFactor * rawHigh * 1.45 + normFlux * 0.18 * (1 - pitchConfidence)`; `noiseFactor = min(1, flatness * 2.5)` | Intended `0..1`; unclamped `normFlux` contribution is clamped at target | `sFx += (clamp01(fxTarget) - sFx) * 0.15`; paused/stopped decay `* 0.9` on render copy only | `State.currentFrame`, dashboard, beat classification, `writeModulationBus` | `FX Presence` | `spectralChaos = scaleUnit(frame.t * 0.42 + features.fx * 0.36 + features.brightness * 0.22)`; drives color shifts, temporal background, FX rings | User-facing label fixed; internal field remains legacy/compatibility | Medium |
| `AudioFrame.state` | `src/audio/analyzer.worker.ts` | worker `onmessage`, pass 2 frame loop | Bar-aligned state from `energyRatio >= 0.45 ? HIGH : LOW`; when HIGH, local overrides: `sE < 0.35 -> LOW_DROP`, `sE > 0.95 -> LOW_OVERLOAD`; default state in store is `IDLE` | Enum: `IDLE`, `HIGH`, `LOW`, `LOW_DROP`, `LOW_OVERLOAD` | Bar-level energy state plus immediate `sE` overrides; no numeric smoothing beyond inputs | Dashboard dynamics, shockwave constructor, classic low-mode color shift, playback-ended reset | `Dynamics State` value text | Classic glow hue uses `state.startsWith('LOW')`; shockwaves receive state | Mostly correct, but `LOW_OVERLOAD` under `LOW` naming is confusing | Medium |
...
| Field name | Owner module | Producer function | Exact formula or derivation | Normalization range | Smoothing | Consumers | Dashboard label | Renderer usage | Semantically correct? | Refactor risk |
|---|---|---|---|---|---|---|---|---|---|---|
| `VisualFeatureFrame.melody` | `src/audio/analyzer.worker.ts` | worker `onmessage`, pass 2 frame loop | Same `sMelody` derivation as `AudioFrame.m`: tonal mid energy plus pitched transient | Intended `0..1` | `0.1` smoothing; paused/stopped decay `* 0.9` | Cues, sections, patterns, modulation, temporal effect | None; Melody Presence is the dashboard-facing melody metric via `AudioFrame.m` | Temporal hue shift, melody ring, `kineticTension`, cue generation | Mostly correct as internal/canonical heuristic presence | Medium |
...
| `BarAnalysis.state` | worker | `barAnalyses` map | `energy >= 0.45 ? HIGH : LOW` | Enum `HIGH | LOW` | Bar-level | Timeline tooltip | Timeline tooltip state | None | Yes | Low |
...
| `State.currentTime` | audio/visual playback sync | `PlexusRenderer.draw` | `engine.getCurrentTime()`; governed by audio engine playback formula | Seconds | Per draw update | Dashboard progress, timeline, renderer frame index | `Progress`, time text | Frame/cue/event indexing, section lookup, pattern resonance | Yes | Low |
...
| `State.modulation.densityDrive` | visuals/config | `writeModulationBus` | `scaleUnit(frame.b * 0.62 + features.density * 0.24 + frame.e * 0.14, sensitivity)` | `0..1` | Input smoothing | Classic/temporal effects, particles | None | Particle speed/Activity Turn input, glow size/alpha, network distance, ring radius | Yes: density is the dominant source and energy is secondary | Medium |
| `State.modulation.spectralChaos` | visuals/config | `writeModulationBus` | `scaleUnit(frame.t * 0.42 + features.fx * 0.36 + features.brightness * 0.22, sensitivity)` | `0..1` | Input smoothing | Classic/temporal effects | None | Line color shifts, temporal background, particle movement, FX ring | Mostly correct | Medium |
...
|---|---|---|---|---|---|---|---|---|---|---|
| `BPM` | `State.bpm` | audio publishes worker result | Worker BPM estimation | Mode of rounded intervals from accepted flux peaks between 70 and 180 BPM; default `120` | BPM number | None | Dashboard only | Beat/bar derivation already done in worker; renderer does not use `State.bpm` except UI timeline bar grid | Yes | Low |
...
| `Density` | `State.currentFrame.b` | UI projection | `DashboardUI.updateDashboard` | Displays density projection as `toFixed(2)` and bar width `b * 100%` | Assumes `0..1` | Source smoothing | User dashboard | Renderer uses via `densityDrive` | Yes at dashboard layer; TypeScript field remains legacy | Medium |
| `Melody Presence` | `State.currentFrame.m` | UI projection | `DashboardUI.updateDashboard` | Displays melody-presence projection as `toFixed(2)` and bar width `m * 100%` | Assumes `0..1` | Source smoothing | User dashboard | No direct effect-module usage | Yes at dashboard layer; TypeScript field remains legacy | Medium |
| `FX Presence` | `State.currentFrame.t` | UI projection | `DashboardUI.updateDashboard` | Displays FX-presence projection as `toFixed(2)` and bar width `t * 100%` | Assumes `0..1` | Source smoothing | User dashboard | Renderer uses via `spectralChaos` | Yes at dashboard layer; TypeScript field remains legacy | Medium |
...
| `Progress` | `State.currentTime / State.duration` | UI projection | `DashboardUI.updateDashboard` | `floor((currentTime / duration) * 100)` and bar width percent | `0..100%` when duration valid | Per dashboard update every fourth draw | User dashboard and seek UI | Renderer uses `currentTime` independently for indexing | Yes | Low |
...
- `ClassicPlexusEffect` does not read raw `AudioFrame.b/m/t` directly. It reads `State.modulation` plus `State.currentFrame.state` and `State.denseImpactFlash`.
...
1. `AudioFrame.b/m/t` are compatibility projections, not spectral bands. The worker still computes raw `rawBassT/rawMidT/rawHighT`, but those are only published at bar level as `BarAnalysis.bass/mid/treble`.
2. The dashboard label issue is fixed: the user-facing labels now read `Density`, `Melody Presence`, and `FX Presence` while TypeScript fields remain unchanged for compatibility.
...
- Fixed: dashboard projection labels were renamed to semantically accurate user-facing labels: Density, Melody Presence, FX Presence, Beat Impulse, and Dynamics State.
...
- High: `AudioFrame.b/m/t` remain legacy internal field names.
...
1. Document `AudioFrame.b/m/t` as legacy compatibility projections in `src/types/index.ts`.
