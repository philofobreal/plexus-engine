...
The visual lens center stays stable, with no camera shake, roll, or horizon jump. `wormholeWarp` means local per-grain spiral/advection amount, and `wormholeCurve` means local per-grain flow curvature: each grain's swirl phase advances with its own forward depth progress, never with wall-clock time. Neither one bends the global route. `wormholePathBend` controls viewer route-following / cosmic turn cue, not a screen-space tube bend, and scales only the sampled canonical arc amplitude. It does not change route radius, topology, or tangent cadence. The route cue is deliberately split into two independent mechanisms:

- **`grainRouteRelative` (core stabilization).** `sampleWormholeRoute(routeDistance, depthT, bend)` returns deterministic `{ offsetX, offsetY, tangentX, tangentY }` values from one explicit distance-domain circular arc route. Grain heads sample `distanceNow + z`; trail tails independently sample `distancePrev + prevZ`. `depthT` applies only a mild lens/far-plane amplitude fade over that same centerline, so it cannot create local S-turns or a separate route per grain. Both endpoints subtract current/previous viewer samples (`wormholeViewerRelativeRoute()`) at canonical travel distance plus fixed lookahead, then apply bounded tangent-heading compensation. The tunnel core therefore stays organized ahead of the viewer while the world turns in the viewer-local frame. Bend zero returns the exact straight baseline.
- **Background viewer-frame world transform (cosmic parallax cue).** Stars, galaxies, and the optional skybox use one shared `sampleWormholeBackgroundViewerFrame(routeDistance, bend)` per frame, not per object. That frame carries the viewer's lateral arc offset and the same arc's tangent. `wormholeBackgroundWorldRelative(worldX, worldY, viewerFrame, worldScale)` then performs the actual per-object transform **in world units, before that layer's own perspective divide**: translate by the viewer's own arc offset without rotating the whole cosmos. Current and previous background endpoints use different viewer frames, so star trail motion correlates with the same route tangent as the foreground arc. Because `sampleWormholeBackgroundViewerFrame` is a pure function of `routeDistance` and `bend`, this needs no mutable per-frame accumulator: seeking to any timestamp reproduces the identical frame without replaying intervening frames. This always reads effective live `wormholePathBend`, never a grain's frozen `releasePathBend`, so a preset/tuning change reaches the background on the very next frame instead of waiting for a grain generation release.

The scene is wrapped by a starfield plus a deeper layer of large, faint `radialGlow` galaxies (gated by `shouldUseExpensiveGlow`), plus an optional skybox. All three read the same shared per-frame viewer frame described above, each with its own world-scale weight (stars strongest, galaxies softer and wider, the skybox faintest since it has no independent depth to divide by and instead scales its translate by a small fraction of its own tile radius); a single master `BACKGROUND_ROUTE_FOLLOW_SCALE` constant scales all three together. No background layer invents an independent random route. Stars and galaxies also fade alpha through their own near-plane zone via `wormholeNearPlaneVisibility()` and floor their own projection depth. `syncPosition()` reconstructs the canonical coordinate after seek/stop, so identical song positions reproduce identical geometry. `wormholeDepthCoherence` can deterministically compress immutable depth cohorts for authored rib character without restoring mutable depth damage; `wormholeRing` separately blends live depths toward discrete rings.
...
- `wormholePathBend` — controls viewer route-following / cosmic turn cue (`0` is the exact straight viewer-frame baseline for both grains and backgrounds), without shake, roll, or horizon jumps. It is an amplitude scalar over one canonical distance-domain arc and never changes route radius, topology, or tangent cadence. Grains use `grainRouteRelative` for core stabilization; stars/galaxies/skybox read a shared, once-per-frame `sampleWormholeBackgroundViewerFrame` and apply it as a translate-only world transform (`wormholeBackgroundWorldRelative`) before each layer's own perspective divide. Always reads effective live tuning, never a grain's frozen release snapshot.
...
presets pin `visualMode` to `cosmic-wormhole`, explicitly carry their route/grain wormhole role keys, and leave background masters global.
