...
- `wormholeStarfield`, `wormholeGalaxy`, `wormholeSkybox` — user-global background layer masters for star density, the deep galaxy layer, and the optional skybox. Current establish/drive/sparse/drift presets explicitly write starfield and galaxy; all omit skybox. Therefore only omitted masters retain their prior value. This differs from the original user-global preset constraint; see [current preset tuning](wormhole-clip-profile.md#current-preset-tuning). Automation-triggered presets with an explicit foreign `visualMode` are filtered by the identity tuning registry before they can write wormhole-owned keys. Manual preset loading and `visualMode`-less presets remain backward compatible.
...
## Skybox heading and spiral interpolation

The skybox uses the canonical route camera heading without a second turn-intensity smoothing
state. Horizontal heading rotates its plate points around the plate center with smooth
`tanh` saturation; vertical heading pans them. `SKYBOX_ROUTE_WORLD_FRACTION` is 0.15.
This dome behavior is local to the skybox, not a rotation of the canvas, foreground, stars
or galaxies. Current/previous canonical camera frames determine both ends of a trail.
Straight flight has a nonzero forward cue: the skybox rate cap is 60 and the relative
forward separation cap is 0.05. Long sustained turns remain bounded; no wall clock or
frame counter accumulates displacement.

Spiral-arm count is clamped to 0..6 but is continuous during tuning morphs, avoiding
integer jumps in the angular material frequency. Integer-authored endpoints still produce
the selected whole-arm character. This does not introduce frame-history interpolation.

Validation: [integration audit](../audits/mvp-renderer-integration-audit.md).
