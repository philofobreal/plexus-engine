...
test('spiral and overdrive keep foreground vanishing point lens-local while bending orientation', () => {
  const load = createSourceLoader();
  const { CosmicWormholeIdentity } = load('visuals/CosmicWormholeIdentity.ts');
  const { State } = load('state/store.ts');
  setupReleaseTestState(State, []);

  const presetNames = ['vos-wh-spiral.json', 'vos-wh-overdrive.json'];
...
test('stateful cosmos route sampling keeps skybox O(1) and item layers O(item-count)', () => {
  const source = readFileSync(join(process.cwd(), 'src/visuals/CosmicWormholeIdentity.ts'), 'utf8');
  const skyStart = source.indexOf('    private drawSkybox(');
  const skyEnd = source.indexOf('\n    }', skyStart);
  const skyBody = source.slice(skyStart, skyEnd);
  assert.doesNotMatch(skyBody, /routePath\.sample/, '9000-star skybox must use shared frames only');

  const galaxyLoop = source.slice(
    source.indexOf('for (let i = 0; i < this.galaxyPool.length; i++)'),
    source.indexOf('const starAmount = tuning.wormholeStarfield')
  );
  assert.equal((galaxyLoop.match(/routePath\.sample/g) ?? []).length, 2, 'galaxy owns current/previous samples per item');

  const starLoop = source.slice(
    source.indexOf('for (let i = 0; i < this.starPool.length; i++)'),
    source.indexOf('// --- Color:')
  );
  assert.equal(
    (starLoop.match(/routePath\.sample/g) ?? []).length,
    2,
    'star owns current/previous route samples per item'
  );
});
